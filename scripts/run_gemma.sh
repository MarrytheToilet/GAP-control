...
say "[gemma] build prefixes"; run scripts/build_prefixes.py --config configs/$T.yaml
say "[gemma] compute steering (CAA)"; run scripts/compute_steering.py --config configs/$T.yaml --pairs
say "[gemma] estimate teacher"; run scripts/estimate_teacher_multi.py --config configs/$T.yaml
mkdir -p models/controller/$T
say "[gemma] train full"; run scripts/train_compositional.py --config configs/${T}_full.yaml --out models/controller/$T/full.pt
say "[gemma] train lmsteer"; run scripts/train_compositional.py --config configs/${T}_lmsteer.yaml --out models/controller/$T/lmsteer.pt
say "[gemma] train caa"; run scripts/train_compositional.py --config configs/${T}_caa.yaml --out models/controller/$T/caa.pt
...
for SEED in 1 2 3; do
  say "[gemma] decode seed=$SEED"
  dec --config configs/_gm_pp_single.yaml --ckpt $GAP --methods base --prompts $P --seed $SEED --out outputs/mm_gem_multi_base_s$SEED.jsonl
  for C in single id2 ho2 tri; do
    dec --config configs/_gm_pp_$C.yaml  --ckpt $GAP --methods prompting,preadd --prompts $P --seed $SEED --out outputs/mm_gem_multi_pp_${C}_s$SEED.jsonl
    dec --config configs/_gm_gap_$C.yaml --ckpt $GAP --methods gap --prompts $P --seed $SEED --out outputs/mm_gem_multi_gap_${C}_s$SEED.jsonl
    dec --config configs/_gm_lm_$C.yaml  --ckpt $LM  --methods gap --prompts $P --seed $SEED --out outputs/mm_gem_multi_lm_${C}_s$SEED.jsonl
  done
done
say "ALL GEMMA DONE"
